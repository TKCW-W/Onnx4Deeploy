...
"""exp4 SR control — stochastic rounding under the CURRENT canonical protocol (lr 3e-6, round 1).
...
def main():
    S.SEED = 42
    torch.manual_seed(12345)      # seed the SR Bernoulli draws for reproducibility
    r = S.load_results()
    ...
    S.log(f"[sr] zero-shot b2 = {zs:.2f}%   lr={LR:g}  steps={STEPS}")
    ...
    for u in range(STEPS):
        ...
        for p in params:
            ...
            if p["kind"] == "float":
                ...
                fl = torch.floor(uu); delta = fl + (torch.rand_like(uu) < (uu - fl)).to(uu.dtype)  # STOCHASTIC ROUNDING
        ...
        if (u + 1) % 675 == 0:
            ...
            S.log(f"  [sr] step {u+1}/{STEPS} b2={b:.2f}% "
    ...
    S.log(f"[sr] FINAL b2={bal:.2f}% (zero-shot {zs:.2f})  "
