...
# QW: these DeepQuant symbols were imported but never used here ("RequantShift" only
#     appears as a string op_type literal), so the import is dropped to keep non-quant
...
def inject_perturbation_nodes(
    ...
    def modify_graph(original_model: onnx.ModelProto, output_path: str, exceptions: list[str]):
        ...
        for node in original_model.graph.node:
            ...
            # QW: added "BatchNormalization" so SpeechNet trains its BN affine (γ/β) via ZO while the
            #     running stats stay frozen. -- QW
            if node.op_type in ["Conv", "Gemm", "MatMul", "RequantShift", "BatchNormalization"] and node.name not in exceptions:
                ...
                for i, input_name in enumerate(node.input):
                    ...
                    if node.op_type == "RequantShift" and i != 2:
                        continue
                    # QW: for BatchNormalization, perturb ONLY scale (γ, idx 1) and bias (β, idx 2) —
                    #     never running_mean (idx 3) or running_var (idx 4): stats stay frozen. -- QW
                    if node.op_type == "BatchNormalization" and i not in (1, 2):
                        continue
