...
def _exec_standard(op: str, inputs: List, attrs: Dict[str, Any]) -> List[np.ndarray]:  # noqa: C901
    ...
    if op == "BatchNormalization":
        ...
        # QW: params are per-channel (C,); reshape to (1,C,1,...) to broadcast over N,C,H,W (NCHW),
        #     not the last axis. Frozen-stats eval BN (uses the running mean/var initializers). -- QW
        if x.ndim > 1:
            shp = [1, x.shape[1]] + [1] * (x.ndim - 2)
            scale = np.asarray(scale).reshape(shp); bias = np.asarray(bias).reshape(shp)
